...
cJSON * UseAfterFreeBug::getBugDescription() const
{
    ...
    cJSON_AddItemToObject(bugDescription, "UAFPath", pathInfo);
...
{
    SVFUtil::outs() << SVFUtil::bugMsg2("\t Use After Free :") <<  " memory allocation at : ("
                    << GenericBug::getLoc() << ")\n";

    SVFUtil::outs() << "\t\t UAF path: \n";
    auto lastBranchEventIt = bugEventStack.end() - 1;
    for(auto eventIt = bugEventStack.begin(); eventIt != lastBranchEventIt; eventIt++)
    {
        SVFUtil::outs() << "\t\t  --> (" << (*eventIt).getEventLoc() << "|" << (*eventIt).getEventDescription() << ") \n";
...
cJSON * NullPointerDereferenceBug::getBugDescription() const
{
    cJSON *bugDescription = cJSON_CreateObject();
    ...
    cJSON_AddItemToObject(bugDescription, "NPDPath", pathInfo);

    return bugDescription;
}

void NullPointerDereferenceBug::printBugToTerminal() const
{
    SVFUtil::outs() << SVFUtil::bugMsg2("\t Null Pointer Dereference :") // <<  " memory allocation at : ("
                    << GenericBug::getLoc() << ")\n";

    SVFUtil::outs() << "\t\t NPD path: \n";
    auto lastBranchEventIt = bugEventStack.end() - 1;
    for(auto eventIt = bugEventStack.begin(); eventIt != lastBranchEventIt; eventIt++)
    {
        SVFUtil::outs() << "\t\t  --> (" << (*eventIt).getEventLoc() << "|" << (*eventIt).getEventDescription() << ") \n";
